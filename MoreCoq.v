...
Proof.
  ...
  intros H. simpl in H. apply f_equal.
  apply IHn.
  apply H.
Qed.
(** [] *)
...
Theorem app_length_comm: forall (X: Type) (l1 l2: list X),
  length (l1 ++ l2) = length l1 + length l2.
Proof.
  intros X l1.
  induction l1.
  intros l2. reflexivity.
  intros l2. simpl. apply f_equal.
  apply IHl1.
Qed.
...
Proof.
  ...
  induction l.
  intros n eq.
  destruct n. reflexivity. inversion eq.
  intros n eq.
  destruct n. inversion eq.
  simpl in *.
  inversion eq.
  rewrite -> H0.
  replace (length (l ++ x :: l)) with (length l + length (x :: l)).
  simpl.
  rewrite -> H0.
  reflexivity.
  rewrite <- app_length_comm.
  reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  intros l1.
  induction l1.
  intros l2 eq. destruct x. inversion eq.
  intros l2 eq.
  induction l2. destruct x. inversion eq.
  destruct x. inversion eq. simpl. rewrite H1. rewrite H3.
  apply f_equal.
...
Proof.
  intros f b.
  destruct b.
  destruct (f true) eqn: ftrue. rewrite ftrue. apply ftrue.
  destruct (f false) eqn: ffalse. apply ftrue. apply ffalse.
  destruct (f false) eqn: ffalse. destruct (f true) eqn: ftrue.
  apply ftrue. apply ffalse. rewrite ffalse. apply ffalse.
Qed.
(** [] *)
...
Proof.
  ...
  Case "l1 = []".
    intros l2 eq.
    destruct l2. reflexivity. inversion eq.
  Case "l1 = x :: l1".
    intros l2 eq.
    destruct l2.
    SCase "l2 = []". inversion eq.
    SCase "l2 = x0 :: l2".
      inversion eq.
      simpl.
      apply IHl1 in H0.
...
Proof.
  intros X x1 x2 k1 k2 k3 f not_eq.
  unfold override.
  destruct (beq_nat k1 k3) eqn: k1_and_k3.
  apply beq_nat_true in k1_and_k3.
  rewrite <- k1_and_k3.
  rewrite -> not_eq.
  reflexivity.
  reflexivity.
...
Proof.
  ...
  intros lf eq.
  inversion eq.
  destruct (test x0) eqn: t.
  inversion H0.
  rewrite <- t. rewrite H1. reflexivity.
  apply IHl in H0.
  apply H0.
Qed.
(** [] *)
...
Fixpoint forallb {X: Type} (test: X -> bool) (l: list X) :=
  match l with
    | [] => true
    | x :: tl => andb (test x) (forallb test tl)
  end.

Fixpoint existb {X: Type} (test: X -> bool) (l: list X) :=
  match l with
    | [] => false
    | x :: tl => orb (test x) (existb test tl)
  end.

Definition existb' {X: Type} (test: X -> bool) (l: list X) :=
  negb (forallb (fun x => negb (test x)) l).

Example forall_test_1: forallb evenb [2; 4; 6; 8] = true.
Proof. reflexivity. Qed.

Example forall_test_2: forallb evenb [2; 4; 6; 8; 9] = false.
Proof. reflexivity. Qed.

Example exist_test_1: existb evenb [1; 3; 6; 9] = true.
Proof. reflexivity. Qed.

Example exist'_test_1: existb' evenb [1; 3; 6; 9] = true.
Proof. reflexivity. Qed.

Theorem exist_eq_exist': forall (X: Type) (l: list X) (f: X -> bool),
...
Proof.
  ...
  destruct (f x) eqn: fx.
  simpl.
  replace (negb (f x)) with false.
  reflexivity.
  rewrite fx.
  reflexivity.
  simpl.
  replace (negb (f x)) with true.
  reflexivity.
  rewrite fx.
  reflexivity.
Qed.
