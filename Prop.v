...
Proof.
  ...
  replace 13 with (5 + 8).
  apply g_plus5.
  replace (S (S (S (S (S (S (S (S n)))))))) with (5 + 3 + n).
  apply g_plus5.
...
Proof.
  ...
  induction n.
  apply ev_0.
  simpl. apply ev_SS with (n := double n).
  apply IHn.
Qed.
(** [] *)
...
Proof.
  ...
  rewrite <- double_plus in enm.
  apply ev_ev__ev with (n := double n) (m := m + p) in enm.
  apply enm.
  apply double_even.
  rewrite <- plus_assoc with (m := n).
...
Inductive pal {X: Type} : list X -> Prop :=
  ...
  | p_sym : forall (x: X) (l: list X), pal l -> pal (snoc (x :: l) x).

Theorem pal_l_append_rev_l : forall (X: Type) (l: list X),
  pal (l ++ rev l).
Proof.
  intros X l.
  induction l.
  ...
  apply p_sym. apply IHl.
Qed.

Theorem pal_l_eq_rev_l: forall (X: Type) (l: list X),
  pal l -> l = rev l.
Proof.
  intros X l p.
  induction p.
  reflexivity.
  reflexivity.
  simpl. rewrite rev_snoc. rewrite <- IHp. reflexivity.
Qed.
...
Theorem l_eq_rev_l_pal: forall (X: Type) (l: list X),
  l = rev l -> pal l.
Proof.
  intros X l eq.
  induction l.
...
Inductive subseq : list nat -> list nat -> Prop :=
    s_nil: forall (l: list nat), subseq [] l
  | s_app: forall (n: nat) (l1 l2: list nat), subseq l1 l2 -> subseq (n :: l1) (n :: l2)
  | s_add: forall (n: nat) (l1 l2: list nat), subseq l1 l2 -> subseq l1 (n :: l2).

Theorem subseq_ref: forall (l: list nat),
  subseq l l.
Proof.
  intros l.
  induction l.
...
Theorem subseq_l1_l2_and_l2_app_l3: forall (l1 l2 l3: list nat),
...
Proof.
  ...
  apply s_nil.
  simpl.
  apply s_app. apply IHs.
  simpl. apply s_add. apply IHs.
Qed.
...
Proof.
  ...
  intros. inversion H. apply s_nil. apply IHsubseq in H3. apply s_app. apply H3.
  apply IHsubseq in H3. apply s_add. apply H3.
  intros. apply IHsubseq in H. apply s_add. apply H.
Qed.
(* FILL IN HERE *)
(** [] *)
