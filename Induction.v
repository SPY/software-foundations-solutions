...
Theorem bin_commute : forall b,
  bin_to_nat (succ_bin b) = 1 + (bin_to_nat b).
Proof.
  intros b.
  induction b as [|b'|b'].
  Case "b = BO". reflexivity.
  Case "b = BT b'". simpl. reflexivity.
  Case "b = BTS b'".
    simpl.
    rewrite -> IHb'.
    simpl.
...
Theorem nat_bin_inverse: forall n,
...
Fixpoint is_normalized (b: bin) : bool :=
  match b with
    | BO => true
    | BT BO => false
    | BT b' => is_normalized b'
    | BTS b' => is_normalized b'
  end.

Fixpoint normalize (b: bin) : bin :=
  match b with
    | BO => BO
    | BT b' => match is_zero b' with
                 | true => BO
                 | false => BT (normalize b')
               end
    | BTS b' => BTS (normalize b')
  end.

Example is_normal_1 : is_normalized BO = true.
Proof. reflexivity. Qed.

Example is_normal_2 : is_normalized (BT BO) = false.
Proof. reflexivity. Qed.

Example is_normal_3 : is_normalized (BT (BTS BO)) = true.
Proof. reflexivity. Qed.

Theorem normal_after_normal: forall b,
  is_normalized (normalize b) = true.
Proof.
  intros b.
  induction b.
  Case "b = BO". reflexivity.
...
Theorem BT_is_normal : forall b,
  is_normalized (BT b) = negb (is_zero b).
Proof.
  intros b.
  induction b.
  Case "b = BO". reflexivity.
  ...
  Case "b = BTS".
    simpl.
    rewrite <- normal_after_normal with b.
Admitted.

Theorem bin_succ_normal : forall b,
  is_normalized b = is_normalized (succ_bin b).
Proof.
  intros b.
  induction b as [| b' | b'].
  Case "b = BO". reflexivity.
  Case "b = BT b'".
    rewrite -> BT_is_normal.
    simpl.
    rewrite -> IHb'.
    simpl.
Admitted.

Theorem nat_to_bin_normal : forall n,
  is_normalized (nat_to_bin n) = true.
Proof.
  intros n.
  induction n.
  ...
  Case "n = S n'".
    rewrite <- IHn.
    simpl.
    rewrite <- bin_succ_normal.
    reflexivity.
Qed.

Theorem bin_nat_inverse : forall b,
