...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
    | nil => nil
    | cons n tl => match evenb n with
                     | true => oddmembers tl
                     | false => n :: oddmembers tl
                   end
  end.
...
Fixpoint alternate (l1 l2 : natlist) : natlist :=
  match l1, l2 with
    | nil, l => l
    | l, nil => l
    | (n1::tl1), (n2::tl2) => n1 :: n2 :: alternate tl1 tl2
  end.
...
Fixpoint count (v:nat) (s:bag) : nat := 
  match s with
    | nil => O
    | cons n tl => match beq_nat v n with
                     | true => 1
                     | false => 0
                   end + count v tl
  end.
...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  match s with
    | nil => nil
    | cons n tl => match beq_nat n v with
                     | true => tl
                     | false => n :: remove_one v tl
                   end
  end.
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
    | nil => true
    | cons n tl => match member n s2 with
                     | true => subset tl (remove_one n s2)
                     | false => false
                   end
  end.
...
(** Write down an interesting theorem about bags involving the
    ...
    stuck! *)
Theorem add_one_to_bag: forall (n: nat) (b: bag),
...
Proof.
  ...
  Case "l = cons n l".
    simpl.
    assert (H: forall l', rev (snoc l' n) = n :: (rev l')).
    SCase "Proof of H". intros l'.
      induction l'.
      reflexivity.
      simpl. rewrite -> IHl'. reflexivity.
    rewrite -> H.
    assert (H': n :: l = n :: rev (rev l)).
...
Proof.
  ...
  Case "l1 = n :: l1".
    ...
    simpl.
    rewrite -> snoc_append.
    replace (snoc (rev l1) n) with (rev l1 ++ [n]).
    rewrite -> app_ass.
    reflexivity.
    rewrite -> snoc_append.
    reflexivity.
...
Lemma cons_eq_singl_app: forall (n: nat) (l: natlist),
  n :: l = [n] ++ l.
Proof.
  intros.
  induction l.
...
Theorem count_on_cons : forall (n m: nat) (s: bag),
  count n (m :: s) = count n [m] + count n s.
Proof.
  intros.
  induction s.
...
Theorem count_sum_dist: forall (n: nat) (s1 s2: bag),
...
Proof.
  ...
  Case "s1 = []". reflexivity.
  Case "s1 = n :: s1'".
    assert (H: count n (n0 :: s1) = count n [n0] + count n s1).
      simpl. rewrite <- plus_assoc.
      replace (0 + count n s1) with (count n s1).
      reflexivity.
      reflexivity.
    rewrite -> H.
    replace (sum (n0 :: s1) s2) with (n0 :: sum s1 s2).
    replace (count n (n0 :: sum s1 s2)) with
      (count n [n0] + count n (sum s1 s2)).
    rewrite <- IHs1.
    rewrite <- plus_assoc.
    reflexivity.
    rewrite <- count_on_cons.
    reflexivity.
    reflexivity.
Qed.
...
*)
Theorem rev_infective : forall (l1 l2 : natlist),
  rev l1 = rev l2 -> l1 = l2.
Proof.
  intros.
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  match l1, l2 with
    ...
    | nil, _ => false
    | _, nil => false
    | n1 :: t1, n2 :: t2 => if beq_nat n1 n2 then beq_natlist t1 t2 else false
  end.
