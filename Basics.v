...
Fixpoint bin_to_nat (b: bin) : nat :=
  match b with
    | BO => O
    | BT b' => 2 * (bin_to_nat b')
    | BTS b' => 1 + (2 * (bin_to_nat b'))
  end.

Example bin_test_1 : bin_to_nat (BTS (BTS BO)) = 3.
Proof. simpl. reflexivity. Qed.

Example bin_test_2 : bin_to_nat (succ_bin (BTS (BTS BO))) = 4.
Proof. simpl. reflexivity. Qed.

Example bin_test_3 : bin_to_nat (succ_bin (BTS (BTS (BTS BO)))) = 8.
Proof. simpl. reflexivity. Qed.

(*
Theorem succ_and_bin_to_nat_assoc: forall (b: bin),
  bin_to_nat b + 1 = bin_to_nat (succ_bin b).
Proof.
  intros b.
  induction b.
  reflexivity.
